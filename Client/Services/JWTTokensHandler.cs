...
public class JWTTokensHandler : DelegatingHandler
{
	...
	protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
	{
		...
		if(!Routes.PointsToIdentity(request.RequestUri?.AbsolutePath))
		{
			...
			if(timeToExpiry.TotalSeconds < 60)
			{
				...
				if(!refreshResponse.IsSuccessStatusCode)
					throw new Exception($"Refresh failed with {(int)refreshResponse.StatusCode} ({refreshResponse.StatusCode})");
